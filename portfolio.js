    ...
    <nav class="navbar navbar-expand-lg navbar-light">
        <div class="container">
            <a class="navbar-brand" href="index.html">
                <img src="Logo K2S 1.png" alt="Logo de Khelcom Services Sénégal" class="logo">
            </a>
...
const portfolioItems = [
    ...
    },
    {
        ...
        image: 'fora 4.jpg',
        title: 'Mini-forage agricole',
        description: 'Réalisation d\'un forage pour un accès à l\'eau potable dans une zone rurale du Sénégal.'
    },
    {
        category: 'solaire',
...
// Génération dynamique du contenu portfolio
const generatePortfolio = () => {
    const portfolioGrid = document.querySelector(".portfolio-grid");
    portfolioItems.forEach(item => {
        const portfolioCard = document.createElement("div");
        portfolioCard.classList.add("col-lg-4", "col-md-6", "mb-4");

        portfolioCard.innerHTML = `
            <div class="portfolio-card">
                <img src="${item.image}" alt="${item.title} - ${item.description}" class="img-fluid">
                <h3>${item.title}</h3>
                <p>${item.description}</p>
            </div>
        `;
